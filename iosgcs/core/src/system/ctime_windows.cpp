...
void CTimePrivate::captureTime()
{
	...
	{
		return;
		hlassert_x(false, "Error calling QueryPerformanceFrequency");
	}
	...
	{
		return;
		hlassert_x(false, "Error calling QueryPerformanceCounter");
	}
...
{
	hlassert_x(m_bIsValid, "Invalid end time sample");
	hlassert_x(start.m_bIsValid, "Invalid start time sample");

	if (m_freq.QuadPart != start.m_freq.QuadPart)
	{
		hlassert_x(false, "Samples were taken at different frequencies");
		return -1;
	}
