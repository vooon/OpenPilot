...
#define IS(Type, Class)	(((opuint32)1 << (Type)) & (Class))
#define OR(Type, Rest)	(((opuint32)1 << (Type)) | (Rest))
 ...
 **/
bool
g_unichar_isdefined (opuint32 c)
{
	return !IS (TYPE(c),
				OR (G_UNICODE_UNASSIGNED,
					OR (G_UNICODE_SURROGATE,
 ...
 **/
bool
g_unichar_iszerowidth (opuint32 c)
{
 ...
 */
bool
g_unichar_iswide_cjk (opuint32 c)
{
 ...
 **/
opuint32
g_unichar_toupper (opuint32 c)
{
	int t = TYPE (c);
	if (t == G_UNICODE_LOWERCASE_LETTER)
    {
		opuint32 val = ATTTABLE (c >> 8, c & 0xff);
		if (val >= 0x1000000)
		{
 ...
 **/
int
g_unichar_digit_value (opuint32 c)
{
 ...
 **/
int
g_unichar_xdigit_value (opuint32 c)
{
	if (c >= 'A' && c <= 'F')
		return c - 'A' + 10;
	if (c >= 'a' && c <= 'f')
			  ...
			  LocaleType   locale_type)
{
	...
	while ((int(max_len) < 0 || p < str + max_len) && *p)
    {
		...
		{
			size_t char_len = g_utf8_skip[*(opuint8 *)last];
			  ...
			  LocaleType   locale_type)
{
	...
	while ((int(max_len) < 0 || p < str + max_len) && *p)
    {
		...
		else if (c == 0x03A3)	/* GREEK CAPITAL LETTER SIGMA */
		{
			...
			{
				opuint32 next_c = g_utf8_get_char (p);
