 ...
 **/
int
g_unichar_combining_class (opuint32 uc)
{
 ...
 **/
void
g_unicode_canonical_ordering (opuint32 *string,
							  size_t     len)
{
	size_t i;
	int swap = 1;
                  ...
                  size_t *result_len)
{
    ...
    {
		opuint32 L = LBase + SIndex / NCount;
		opuint32 V = VBase + (SIndex % NCount) / TCount;
		opuint32 T = TBase + SIndex % TCount;
...
static const char *
find_decomposition (opuint32 ch,
					bool compat)
{
	int start = 0;
 ...
 **/
opuint32 *
g_unicode_canonical_decomposition (opuint32 ch,
								   size_t   *result_len)
{
	const char *decomp;
	const char *p;
	...
	if (ch >= 0xac00 && ch <= 0xd7a3)
    {
		...
		r = (opuint32*)malloc (*result_len * sizeof (opuint32));
    ...
    {
		/* Not in our table.  */
		r = (opuint32*)malloc (sizeof (opuint32));
		*r = ch;
		*result_len = 1;
    }
