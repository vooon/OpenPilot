 ...
 * \return -1 Failure
 */
int32_t UAVTalkSendObject(UAVObjHandle obj, uint16_t instId, uint8_t acked,
			  int32_t timeoutMs) {
  // Send object
  if (acked == 1) {
 ...
 * \return -1 Failure
 */
static int32_t objectTransaction(UAVObjHandle obj, uint16_t instId,
                                 uint8_t type, int32_t timeoutMs) {
 ...
 * \return -1 Failure
 */
int32_t UAVTalkProcessInputStream(uint8_t rxbyte_in) {
  ...
  case STATE_SYNC:
    ...
    cs = PIOS_CRCupdateByte(0, rxbyte);
    rxPacketLength = 1;
    state = STATE_TYPE;
    break;
  ...
  case STATE_TYPE:
    ...
    // update the CRC
    cs = PIOS_CRCupdateByte(cs, rxbyte);
  ...
  case STATE_SIZE:
    ...
    if (packet_size < MIN_HEADER_LENGTH ||
	packet_size > MAX_HEADER_LENGTH + MAX_PAYLOAD_LENGTH) {
      state = STATE_SYNC;
      break;
    }
  ...
  case STATE_CS:
    ...
    if (csRx != cs) {   // packet error - faulty CRC
      stats.rxErrors++;
      state = STATE_SYNC;
      break;
    }
 ...
 * \return -1 Failure
 */
static int32_t receiveObject(uint8_t type, uint32_t objId, uint16_t instId,
 ...
 * \return -1 Failure
 */
static int32_t sendSingleObject(UAVObjHandle obj, uint16_t instId,
				uint8_t type) {
  int32_t length;
  int32_t dataOffset;
  uint32_t objId;
...
static int32_t sendNack(uint32_t objId) {
  ...
  // Calculate checksum
  txBuffer[dataOffset] = PIOS_CRCupdateCRC(0, txBuffer, dataOffset);
