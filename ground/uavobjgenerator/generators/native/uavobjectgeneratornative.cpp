...
#include "uavobjectgeneratornative.h"

using namespace std;

bool UAVObjectGeneratorNative::generate(UAVObjectParser* parser,QString templatepath,QString outputpath) {

    fieldTypeStrCPP << "opint8" << "opint16" << "opint32" <<
        "opuint8" << "opuint16" << "opuint32" << "float" << "opuint8";

    fieldTypeStrCPPClass << "INT8" << "INT16" << "INT32"
        << "UINT8" << "UINT16" << "UINT32" << "FLOAT32" << "ENUM";

    gcsCodePath = QDir( templatepath + QString(NATIVE_CODE_DIR));
    gcsOutputPath = QDir( outputpath + QString("native") );
 ...
 */
bool UAVObjectGeneratorNative::process_object(ObjectInfo* info)
{
    if (info == NULL)
        return false;
    ...
    for (int n = 0; n < info->fields.length(); ++n)
    {
        ...
        if ( field->numElements > 1 ) {
            propertyGetters +=
                    QString("    %1 get%2(opuint32 index) const;\n")
                    .arg(type).arg(field->name);
            propertiesImpl +=
                    QString("%1 %2::get%3(opuint32 index) const\n"
                            "{\n"
            ...
            propertySetters +=
                    QString("    void set%1(opuint32 index, %2 value);\n")
                    .arg(field->name).arg(type);
            propertiesImpl +=
                    QString("void %1::set%2(opuint32 index, %3 value)\n"
                            "{\n"
                            "   m_mutex.lock();\n"
                            "   bool changed = data.%2[index] != value;\n"
                            "   data.%2[index] = value;\n"
                            "   m_mutex.unlock();\n"
                            "   if (changed) m_signal_%2Changed.invoke(index,value);\n"
                            "}\n\n")
                    .arg(info->name).arg(field->name).arg(type);
            propertyNotifications +=
                    QString("   CL_Signal_v2<opuint32 /*index*/, %1 /*value*/> m_signal_%2Changed;")
                        .arg(type).arg(field->name);
//                    QString("    void %1Changed(quint32 index, %2 value);\n")
//                    .arg(field->name).arg(type);
        } else {
            propertyGetters +=
                    QString("    %1 get%2() const;\n")
                    .arg(type).arg(field->name);
            propertiesImpl +=
            ...
            propertiesImpl +=
                    QString("void %1::set%2(%3 value)\n"
                            ...
                            "   data.%2 = value;\n"
                            "   m_mutex.unlock();\n"
                            "   if (changed) m_signal_%2Changed.invoke(value);\n"
                            "}\n\n")
                    .arg(info->name).arg(field->name).arg(type);
            propertyNotifications +=
                    QString("   CL_Signal_v1<%1 /*value*/> m_signal_%2Changed;\n")
                        .arg(type).arg(field->name);
//                    QString("    void %1Changed(%2 value);\n")
//                    .arg(field->name).arg(type);
            propertyNotificationsImpl +=
                    QString("   //if (data.%1 != oldData.%1)\n"
                        "     m_signal_%1Changed.invoke(data.%1);\n")
//                    QString("        //if (data.%1 != oldData.%1)\n"
//                            "            emit %1Changed(data.%1);\n")
    ...
    for (int n = 0; n < info->fields.length(); ++n)
    {
        ...
        finit.append( QString("    CStringList %1;\n").arg(varElemName) );
        QStringList elemNames = info->fields[n]->elementNames;
        for (int m = 0; m < elemNames.length(); ++m)
            finit.append( QString("    %1.push_back(\"%2\");\n")
                          .arg(varElemName)
                          .arg(elemNames[m]) );
        ...
        if (info->fields[n]->type == FIELDTYPE_ENUM) {
            ...
            finit.append( QString("    fields.push_back( new UAVObjectField(CString(\"%1\"), CString(\"%2\"), UAVObjectField::ENUM, %3, %4, CString(\"%5\")));\n")
        ...
        // For all other types
            ...
            finit.append( QString("    fields.push_back( new UAVObjectField(CString(\"%1\"), CString(\"%2\"), UAVObjectField::%3, %4, CStringList(), CString(\"%5\")));\n")
                          .arg(info->fields[n]->name)
                          .arg(info->fields[n]->units)
    ...
    for (int n = 0; n < info->fields.length(); ++n)
    {
        ...
        if (info->fields[n]->numElements > 1) {
            ...
            enums.append( QString("    static const opuint32 %1_NUMELEM = %2;\n")
