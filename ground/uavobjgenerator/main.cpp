...
#include "generators/obj-c/uavobjectgeneratorobjc.h"
 ...
 */
void usage() {
    cout << "Usage: uavobjectgenerator [-gcs] [-flight] [-java] [-python] [-matlab] [-objc] [-none] [-v] xml_path template_base [UAVObj1] ... [UAVObjN]" << endl;
    cout << "Languages: "<< endl;
    ...
    cout << "\t-objc          build objective-c code" << endl;
...
int main(int argc, char *argv[])
{
    ...
    bool do_objc=(arguments_stringlist.removeAll("-objc")>0);
    bool do_none=(arguments_stringlist.removeAll("-none")>0); //

    bool do_all=((do_gcs||do_flight||do_java||do_python||do_matlab||do_objc)==false);
    ...
    // generate gcs code if wanted
    if (do_objc|do_all) {
        cout << "generating objc code" << endl ;
        UAVObjectGeneratorObjC objcgen;
        objcgen.generate(parser,templatepath,outputpath);
    }
