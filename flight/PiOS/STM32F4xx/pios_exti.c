...
extern void PIOS_Hsync_ISR(void);
extern void PIOS_Vsync_ISR(void);
...
void EXTI0_IRQHandler(void)
{
	...
	if (EXTI_GetITStatus(PIOS_VIDEO_HSYNC_EXTI_LINE) != RESET) {
		PIOS_Hsync_ISR();
		EXTI_ClearITPendingBit(PIOS_VIDEO_HSYNC_EXTI_LINE);
	}
#endif
...
#if defined(PIOS_INCLUDE_VIDEO)
	if (EXTI_GetITStatus(PIOS_VIDEO_VSYNC_EXTI_LINE) != RESET) {
		PIOS_Vsync_ISR();
		EXTI_ClearITPendingBit(PIOS_VIDEO_VSYNC_EXTI_LINE);
	}
#endif
