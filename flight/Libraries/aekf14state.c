 ...
 * @addtogroup AEKF
 * @{
 * @brief AEKF is a joint attitude and position estimation EKF
 *
 * @file       aekf14state.c
...
#define NUMV 11			// number of measurements, v is the measurement noise vector
#define NUMU 4			// number of deterministic inputs, U is the input vector
...
void AEKFInit()		//pretty much just a place holder for now
{
	Be[0] = 1.0f;
	Be[1] = 0.0f;
	...
	P[0][0] = P[2][2] = P[4][4] = 25.0f;						// initial position variance (m^2)
	P[1][1] = P[3][3] = P[5][5] = 5.0f;							// initial velocity variance (m/s)^2
	P[6][6] = P[7][7] = P[8][8] = P[9][9] = P[10][10] = 1e-5f;	// initial attitude-derived variable variance
	P[11][11] = P[12][12] = P[13][13] = 1e-9f;					// initial gyro variance (rad/s)^2

	X[0] = X[2] = X[4] = 0.0f;	// initial pos [m]
	X[1] = X[3] = X[5] = 0.0f;  // initial vel [m]
	...
	X[10] = X[11] = X[12] = 0.0f;	// initial gyro values (rad/s)

	Qc(7,7)=  1;			%z_7 process noise = 
	Qc(8,8)=  1;			%z_8 process noise = 
	Qc(9,9)=  1;			%z_9 process noise
	Qc(10,10)=1;		%z_10 process noise  = North_dot_dot * M/u1 East_dot_dot * M/u1
	Qc(11,11)=1;			%z_11 process noise = (Down_dot_dot-g) * M/u1
	
	Q[0] = Q[2] = 10;								// positional noise variance (rad/s)^2
	Q[1] = Q[2] = 1000;								// translational velocity noise variance (rad/s)^2
	Q[6] = Q[7] = Q[8] = Q[9] = Q[10] = 0.00001f;	// z process noise (m/s^2)^2
	Q[11] = Q[12] = Q[13] = 1;						// rotational velocity noise variance (rad/s)^2

	R[0] = R[1] = 0.004f;	// High freq GPS horizontal position noise variance [m^2]
	R[2] = .05f;                    // High freq altimeter noise variance [m^2]
	R[8] = R[9] = R[10] = 1*1f;					// accel noise variance [m^2/s^4]
	R[6] = R[7]=1;								// magnetometer unit vector noise variance
	R[8] = R[9] = R[10] = 50e-4f;					// gyro noise variance [rad^2/s^2]
}
