 ...
 * Specifically updates the the @ref AttitudeActual "AttitudeActual" and @ref 
 * AttitudeRaw "AttitudeRaw" settings objects
 * @{
 *
 * @file       attitude.c
 ...
 *
 * Modules have no API, all communication to other modules is done through 
 * UAVObjects.
...
int32_t AttitudeStart(void)
{
	...
	// Start main task
	xTaskCreate(AttitudeTask, (signed char *)"Attitude",
...
int32_t AttitudeInitialize(void)
{
	...
	gpsNew_flag = false;
	glbl = (struct GlobalAttitudeVariables *)
...
static void AttitudeTask(void *parameters)
{
	...
	/** @todo THIS IS BOARD SPECIFIC AND DOES NOT BELONG HERE. Can we put it 
     * in #if defined(PIOS_INCLUDE_ADXL345)? */
	...
	//Store the original filter specs. This is because we currently have a 
	//poor way of calibrating the Premerlani approach
	...
	while (1) {
		...
		} else if (glbl->zero_during_arming &&
	...
	/** @todo CC IS NOT CURRENTLY SUPPORTED BY NAV BRANCH. FORCE AN ALARM 
         * STATE. */
		if (retval != 0 || cc3d_flag == false)
			...
			if (!AttitudeActualReadOnly()) {
				...
				if (glbl->bias_correct_gyro) {
					updateSensorDrift(&accels, &gyros,
							  delT);
				}
...
//                              float gyrosVec[3] = {gyros.x, gyros.y, gyros.z};
		...
		if (gpsNew_flag == true) {
			...
			if (gpsStatus == GPSPOSITION_STATUS_FIX3D) {
				...
				LLA2NED(LL_int, altitude, geoidSeparation,
					gps_NED);
				...
				float alphaPosNorthEast =
				    dT / (dT + tauPosNorthEast);
				float alphaPosDown = dT / (dT + tauPosDown);
				float alphaVelNorthEast =
				    dT / (dT + tauVelNorthEast);
				...
				velocityActualData.East =
				    (1 -
				     alphaVelNorthEast) *
				    velocityActualData.East +
				    alphaVelNorthEast * gpsVelocityData.East;
				velocityActualData.Down =
				    (1 -
				     alphaVelDown) * velocityActualData.Down +
				    alphaVelDown * gpsVelocityData.Down;

				//Complementary filter for position
				positionActualData.North =
				    (1 -
				     alphaPosNorthEast) *
				    (positionActualData.North +
				     (velocityActualData.North * dT)) +
				    alphaPosNorthEast * gps_NED[0];
				positionActualData.East =
				    (1 -
				     alphaPosNorthEast) *
				    (positionActualData.East +
				     (velocityActualData.East * dT)) +
				    alphaPosNorthEast * gps_NED[1];
				positionActualData.Down =
				    (1 -
				     alphaPosDown) * (positionActualData.Down +
						      (velocityActualData.Down *
						       dT)) +
				    alphaPosDown * gps_NED[2];

				//Very slowly use GPS heading data to converge. This is a poor 
				//way of doing things, but will work in the short term for 
				//testing.
				if (fabs(velocityActualData.North) > 3.0f) {
					//Instead of calculating norm, use a gauge approach
					if (fabs(velocityActualData.North) >
					    4.0f
					    || fabs(velocityActualData.East) >
					    3.0f) {
						float heading =
						    atan2f
						    (velocityActualData.East,
			     ...
			     bool cc3d_flag)
{
	...
	if (glbl->trim_requested) {
		if (glbl->trim_samples >= MAX_TRIM_FLIGHT_SAMPLES) {
			...
			ManualControlCommandThrottleGet(&throttle);	// Until flight status
			// indicates airborne
//                      if ((armed == FLIGHTSTATUS_ARMED_ARMED) && (throttle > 0)) {
			glbl->trim_samples++;
			// Store the digitally scaled version since that is what we use for
			// bias
...
static void updateAttitude(float *gyros, float dT)
{
	...
	{			// scoping variables to save memory
		...
		float qdot[4];
		qdot[0] =
		    (-glbl->q[1] * gyros[0] - glbl->q[2] * gyros[1] -
		     glbl->q[3] * gyros[2]) * dT * M_PI / 180 / 2;
		qdot[1] =
		    (glbl->q[0] * gyros[0] - glbl->q[3] * gyros[1] +
		     glbl->q[2] * gyros[2]) * dT * M_PI / 180 / 2;
		qdot[2] =
		    (glbl->q[3] * gyros[0] + glbl->q[0] * gyros[1] -
		     glbl->q[1] * gyros[2]) * dT * M_PI / 180 / 2;
		qdot[3] =
		    (-glbl->q[2] * gyros[0] + glbl->q[1] * gyros[1] +
		     glbl->q[0] * gyros[2]) * dT * M_PI / 180 / 2;
	...
	// Renomalize
	float qmag =
	    sqrtf(powf(glbl->q[0], 2.0f) + powf(glbl->q[1], 2.0f) +
		  powf(glbl->q[2], 2.0f) + powf(glbl->q[3], 2.0f));
...
static void settingsUpdatedCb(UAVObjEvent * objEv)
{
	...
	glbl->gyroGain[0] = glbl->gyroGain[1] = glbl->gyroGain[2] =
	    glbl->gyroGain_ref = attitudeSettings.GyroGain;

	glbl->zero_during_arming =
	    (attitudeSettings.ZeroDuringArming ==
	     ATTITUDESETTINGS_ZERODURINGARMING_TRUE);
	glbl->bias_correct_gyro =
	...
	// @bug not 1000 and not accelbias, copy paste error?
	//Divide by 1000 because `accelbias` is in units of 1000*[m/s^2]
	glbl->accelscale[0] =
	...
	if (attitudeSettings.TrimFlight == ATTITUDESETTINGS_TRIMFLIGHT_START) {
		...
		float a_body[3] = { glbl->trim_accels[0] / glbl->trim_samples,
			glbl->trim_accels[1] / glbl->trim_samples,
			glbl->trim_accels[2] / glbl->trim_samples
		};
