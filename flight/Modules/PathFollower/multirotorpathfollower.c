...
/**
 * @brief Initialize multirotor path following variables
 */
 ...
 uint8_t updateMultirotorDesiredStabilization(FixedWingPathFollowerSettingsData fixedwingpathfollowerSettings)
{
	...
	// Altitude error	
	float altitudeError=-(pathDesired.End[2]-positionActual.Down); //Negative to convert from Down to altitude
...
#define THROTTLE_ILIMIT fixedwingpathfollowerSettings.ThrottlePI[FIXEDWINGPATHFOLLOWERSETTINGS_THROTTLEPI_ILIMIT]
	...
	if (THROTTLE_KI > 0.0f){
		integral->altitudeError=bound(integral->altitudeError+altitudeError*dT,
										 -THROTTLE_ILIMIT/THROTTLE_KI,
										 THROTTLE_ILIMIT/THROTTLE_KI)*(1.0f-1.0f/(1.0f+30.0f/dT));
	}
	
	powerCommand=altitudeError*THROTTLE_KP
		+ integral->altitudeError*THROTTLE_KI;
...
#define PITCHCROSSFEED_MAX fixedwingpathfollowerSettings.VerticalToPitchCrossFeed[FIXEDWINGPATHFOLLOWERSETTINGS_VERTICALTOPITCHCROSSFEED_MAX]
	float altitudeToPitchCommandComponent=bound ( altitudeError* PITCHCROSSFEED_KP,
													  -PITCHCROSSFEED_MIN,
													  PITCHCROSSFEED_MAX);
...
#define HEADINGPI_KP fixedwingpathfollowerSettings.HeadingPI[FIXEDWINGPATHFOLLOWERSETTINGS_HEADINGPI_KP]
	...
	stabDesired.Yaw = bound( ROLLLIMIT_NEUTRAL +
							 yawCommand,
							 ROLLLIMIT_MIN,
							 ROLLLIMIT_MAX);
	
#ifdef SIM_OSX
	fprintf(stderr, " headingError_R: %f, yawCommand: %f\n", headingError_R, yawCommand);
#endif		
	...
	pathDesired.ModeParameters[3]=altitudeError;
//	pathDesired.UID=altitudeError;
