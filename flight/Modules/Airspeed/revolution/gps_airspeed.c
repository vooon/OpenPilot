...
/*				
		//As long as we're at it, let's calculate windspeed, too. 
		// This math comes from the same Premerlani paper
		{
			//This doesn't fit well with the UAVO approach, since we want to have access to the best estimate of airspeed, which happens
			// later. So for now we'll use the airspeed calculated by the GPS, even though it's very, very noisy.
			WindVelocityData windVelData;
			WindVelocityGet(&windVelData);
			
			
			//Calculate residual yaw error between estimated yaw angle and real yaw angle
			float yawErr;
			float S21[2]={gpsVelData.North-gps->gpsVelOld_N, gpsVelData.East-gps->gpsVelOld_E};
			float F21[2]={Rbe[0][0]-gps->RbeCol1_old[0], Rbe[0][1]-gps->RbeCol1_old[1]};
			
			if (S21[1]*F21[0]-S21[0]*F21[1]) //Check the sign of the angles, then perform the dot product and renormalize, finally taking the arcsin
				yawErr= acosf((S21[0]*F21[0]+S21[1]*F21[1])/sqrtf((S21[0]*S21[0]+S21[1]*S21[1])*(F21[0]*F21[0]+F21[1]*F21[1])));
			else 
				yawErr=-acosf((S21[0]*F21[0]+S21[1]*F21[1])/sqrtf((S21[0]*S21[0]+S21[1]*S21[1])*(F21[0]*F21[0]+F21[1]*F21[1])));

			float alpha=1.0f/(1.0f+5.0f/0.1f); //Set a rise time of about 5 seconds.
			windVelData.North=alpha*windVelData.North +
									(1-alpha)*(gps->gpsVelOld_N+gpsVelData.North- (*v_air_GPS)*(cosf(yawErr)*(gps->RbeCol1_old[0]+Rbe[0][0])-sinf(yawErr)*(gps->RbeCol1_old[1]+Rbe[0][1])))/2.0f;
			windVelData.East =alpha*windVelData.East + 
									(1-alpha)*(gps->gpsVelOld_E+gpsVelData.East - (*v_air_GPS)*(sinf(yawErr)*(gps->RbeCol1_old[0]+Rbe[0][0])+cosf(yawErr)*(gps->RbeCol1_old[1]+Rbe[0][1])))/2.0f;
			windVelData.Down = alpha*windVelData.Down +
									(1-alpha)*(gps->gpsVelOld_D+gpsVelData.Down - (*v_air_GPS)*(gps->RbeCol1_old[2]+Rbe[0][2]))/2.0f;
			
			WindVelocitySet(&windVelData);
		}
*/		
